...
public class AbstractReflectiveVisitor implements Visitor {
	...
	public void dispatch(Object object) {
		try {
			...
			// lastly, invoke callAccept ONLY if the element is of Visitable
			// type
	...
	protected Method getMethod(Class<?> c) {
		...
		Class<? extends AbstractReflectiveVisitor> thisclass = getClass();
		// newc is set to the class of the element to be visited (parameter of
		// this method)
		Class<?> newc = c;
		...
		// Try the interfaces of the element if the method has not yet been
		// determined
		if (m == null)
			newc = c;
		while (m == null && newc != null && newc != Object.class) {
		...
		// Lastly, if no method has been found before this, locate the default
		// visit method
		...
		// if a method was found, cache it, keyed by class c, in the methods
		// hashmap
		if (m != null)
			methods.put(c, m);
