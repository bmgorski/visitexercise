...
public class HDOMUtil {
	...
	public static String textFor(Element element) {
		...
		for (int i = 0; i < count; ++i) {
			...
			if (node instanceof Text) {
				...
				if (!("".equals(value)))
					return value; // Return the trim()'d text of the first child
									// text node
			}
		} // for i...
		return "";
	...
	/**
	 * This will look at the children of the element that have a tag name
			...
			if (isMatchingNode(tagNode, tag)) {
				...
				for (int i = 0; i < count; ++i) {
					...
					if (node instanceof Text) {
						String value = trim((Text) node);
						if (!("".equals(value)))
							return value; // Return the trim()'d text of the
	...
	public static String[] textForTag(Element element, String tag) {
		if (!element.hasChildNodes())
			return new String[0]; // Should never happen if we are passed a
									// valid Element node
