 ...
 * <code>Node</code>s in an implementation which supports the DOM Event Model.
 * The interface allows registration and removal of <code>EventListener</code>s
...
public interface EventTarget {
	 ...
	 *            to any <code>EventTarget</code>s beneath them in the tree.
	 *            Events which are bubbling upward through the tree will not
	 *            trigger an <code>EventListener</code> designated to use
	 *            capture.If an <code>EventListener</code> is added to an
	 *            <code>EventTarget</code> which is currently processing an
	 *            event the new listener will not be triggered by the current
	 *            event.
	 */
	 ...
	 *                UNSPECIFIED_EVENT_TYPE: Raised if the <code>Event</code>'s
