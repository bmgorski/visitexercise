 ...
 * <code>TreeWalker</code> is given a filter, before it returns the next node,
 * it applies the filter. If the filter says to accept the node, the Iterator
 * returns it; otherwise, the Iterator looks for the next node and pretends that
 * the node that was rejected was not there.
 * <p>
 * The DOM does not provide any filters. Filter is just an interface that users
